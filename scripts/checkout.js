  ...
  if (isNaN(nombre) || isNaN(apellido) || isNaN(correo) || isNaN(telefono)) {
    ...
    try {
      ...
      async function initialize() {
        try {
          const response = await fetch(
            "https://codec-x7w2.onrender.com/create-payment-intent",
            {
              method: "POST",
          ...
          if (error) {
            ...
            switch (paymentIntent.status) {
              ...
              case "requires_payment_method":
                showModalPopup(
                  "Su pago no fue exitoso, por favor intente nuevamente."
                );
                break;
              default:
      ...
      async function handleSuccessfulPayment(config) {
        ...
        const correo = document.getElementById("email").value;
        const telefono = document.getElementById("phone").value;
        const asistenciaRompehielos = document.querySelector(
          'input[name="asistencia_rompehielos"]:checked'
        ).value;
          ...
          .insert([
            {
              ...
              email: correo,
              telefono,
              asistencia_rompehielos: asistenciaRompehielos,
            },
          ])
          .select("id")
          .single();
        ...
        try {
          ...
          page.drawText(`Apellido: ${apellido}`, {
            x: 20,
          ...
          page.drawText(`Teléfono: ${telefono}`, {
            x: 20,
            y: page.getHeight() - logoDims.height - 120,
            size: 15,
          });
          page.drawText(
            `Asistira al Evento Rompe Hielos: ${asistenciaRompehielos}`,
            { x: 20, y: page.getHeight() - logoDims.height - 140, size: 15 }
          );
          ...
          if (pdfBlob.size > 5000000) {
            alert(
              "El PDF generado es demasiado grande para ser enviado por correo."
            );
            return;
          }
          ...
          reader.onloadend = function () {
            ...
            emailjs
              .send(config.serviceKey, config.templateKey, {
                to_name: nombre,
      ...
      showModalPopup(
        "Error fetching configuration. Please check the server and try again."
      );
    }
  }
});
